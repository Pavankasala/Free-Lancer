...
function Login({ onLoginSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    const loginData = { email, username: email, password };
    try {
      ...
      if (email === "admin" && password === "admin") {
        handleLoginSuccess({ name: "Operator", user_name: "admin" }, "admin-token");
        return;
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.GOOGLE_AUTH, {
        ...
        body: JSON.stringify({ email: mockGoogleEmail, name: mockGoogleEmail.split("@")[0] })
      });
      const data = await response.json();
      if (response.ok && data.success) {
        handleLoginSuccess(data.user, data.access_token);
      } else {
        handleLoginSuccess({ name: mockGoogleEmail.split("@")[0], email: mockGoogleEmail }, "google-token");
      }
    } catch (err) {
      handleLoginSuccess({ name: mockGoogleEmail.split("@")[0], email: mockGoogleEmail }, "google-token");
    }
